...
class BlueFors(Instrument):
    ...
    def __init__(self, name: str,
        ...
        self.add_parameter(name='pressure_vacuum_can',
                           unit='mBar',
                           get_parser=float,
                           get_cmd=lambda: self.get_pressure(
                               channel_vacuum_can),
        ...
        self.add_parameter(name='temperature_50k_plate',
                           unit='K',
                           get_parser=float,
                           get_cmd=lambda: self.get_temperature(
                               channel_50k_plate),
    ...
    def get_temperature(self, channel: int) -> float:
        ...
        file_path = os.path.join(
            self.folder_path, folder_name, 'CH'+str(channel)+' T '+folder_name+'.log')
    ...
    def get_pressure(self, channel: int) -> float:
        ...
        folder_name = date.today().strftime("%y-%m-%d")
        file_path = os.path.join(
            self.folder_path, folder_name, 'maxigauge '+folder_name+'.log')
        ...
        try:
            ...
            self.log.warn(
                'Cannot access log file: {}. Returning np.nan instead of the pressure value.'.format(err))
            return np.nan
        except IndexError as err:
            self.log.warn(
                'Cannot parse log file: {}. Returning np.nan instead of the pressure value.'.format(err))
            return np.nan
